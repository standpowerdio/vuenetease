<template>
    <div class="songlist">
        <div class="songbox" v-for="(item,i) in $store.state.album" :key="i">
            <img :src="item.picUrl">
            <p>{{item.name}}</p>
            <div class="count">{{item.playCount | playcount}}</div>
        </div>
    </div>
</template>
...
export default {
    ...
    filters:{
        playcount(val){
            if(!val) return ''
            if(val/10000 < 1) return val
            else if(val/100000000 < 1) return Math.round(val/1000)/10 + '万'
            else return Math.round(val/10000000)/10 + '亿'
        }
    }
}
</script>
